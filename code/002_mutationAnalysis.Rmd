...
```{r}
load('../data/sciclone_vafs_and_cnv_inputs.rdata')

vaf_table <- do.call(rbind, vaf_list)

ggplot(data=vaf_table) + 
  aes(x=as.factor(chromNum), y=VAF, fill=as.factor(chromNum)) + 
...
Prepare sample grouping and annotation.
```{r}
names <- c("Recurrent_A", 'Recurrent_B', 'Recurrent_C', 'Primary')
# prepare sample grouping
sample.groups <- as.factor(c('R', 'R', 'R', 'P'))
names(sample.groups) <- names

# prepare time-point information for plots
# first recurrence was after two (28-day) cycles of TMZ.
time.points <- c(56,56,56,0)
names(time.points) <- names
...
```{r message=FALSE, warning=FALSE}
set.seed(0)
sc <- sciClone(vafs=vaf_list_clean,
...
sc_vafs <- data.frame(sc@vafs.merged)
```

Remove NA clusters and negative clusters. Preview the results.
```{r}
sc_vafs <- sc_vafs[!is.na(sc_vafs$cluster) & sc_vafs$cluster > 0,]

sc_vafs
```

```{r fig.height=8, fig.width=10}
sc.plot2d(sc, singlePage = TRUE, ylim=40, xlim=40)
```

```{r fig.height=8, fig.width=10}
sc_long <- sc_vafs %>%
             tidyr::gather(sample, VAF, paste0(names, '.vaf')) %>%
             mutate(sample=gsub('.vaf', '', sample, fixed=TRUE))

sc_box <- ggplot(data =  sc_long) + aes(x = as.factor(cluster), y = VAF, color=as.factor(cluster)) +
             geom_boxplot(width=0.5, show.legend=FALSE) + geom_jitter(alpha=0.7) + facet_wrap(~sample, ncol=1) +
             theme_classic() + scale_color_manual(values=clone.colors)
...
```{r}
set.seed(0)
y = infer.clonal.models(variants = sc_vafs,
                        vaf.col.names = paste0(names, '.vaf'),
...
plot.tree.clone.as.branch(clonal_models$matched$merged.trees[[1]], branch.width =0.5,
                          node.size = 1.5, node.label.size = 0.5,
                          tree.rotation=180)
```
